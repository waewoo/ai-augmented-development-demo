...
La version Slidev verrouillée dans `package.json` fournit également un export `pptx-editable`. Les scripts de diffusion génèrent une copie temporaire sans directives `v-click` afin que le PDF, le PPTX et les PNG restent à 15 pages ; la source `slides.md` conserve les animations pour la présentation web. Il reste conseillé de vérifier le rendu et le degré d’éditabilité dans PowerPoint ou LibreOffice avant diffusion.
